<?php	
	...
	while (($line = fgetcsv($file)) !== FALSE) {
		array_push($exigibles,preg_replace( "/\r|\n|\\\\/", "", $line ));
	}
	fclose($file);
	...
	$firmes = array();
	while (($line = fgetcsv($file)) !== FALSE) {
		array_push($firmes,preg_replace( "/\r|\n|\\\\/", "", $line ));
	}
	fclose($file);
	...
	$noLocalizados = array();
	while (($line = fgetcsv($file)) !== FALSE) {
		array_push($noLocalizados,preg_replace( "/\r|\n|\\\\/", "", $line ));
	}
	fclose($file);
	...
	if( !empty($aExigibles) || !empty($aFirmes) || !empty($aNoLocalizados)  ){
		...
		$sql = $db->prepare('SELECT correo FROM correos_alerta WHERE borrado = 0');
		$sql->execute();
		...
		if( !empty($aNoLocalizados) ){
			$mensaje .= "<p><b>Los siguientes RFC se encontraron en la lista de No Localizados:</b></p>";
			$mensaje .= "<ul>";
